        ...
        <div class="bg-white p-6 md:p-8">
            <div class="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
                ...
                <div class="flex gap-2">
                    <form action="{{ route('admin.materials') }}" method="GET" class="flex">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        @forelse ($allMateri as $materi)
                            <tr >
                                <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{{ $materi->title }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ Str::limit($materi->description, 60) }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    <span class="inline-flex rounded-full px-2 text-xs font-semibold leading-5
                                        {{ $materi->status === 'published' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                        {{ ucfirst($materi->status) }}
                                    </span>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ $materi->duration }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ $materi->file_size }}</td>
